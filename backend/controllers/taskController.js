  ...
  try {
    ...
    if (req.body.assignedTo) {
      if (!Array.isArray(req.body.assignedTo)) {
        return res
          .status(400)
          .json({ message: "assignedTo must be an unique IDs" });
      }
...
const updateTaskChecklist = async (req, res) => {
  try {
    ...
    //auto-mark task is completed if all items are checked
...
const getUserDashboardData = async (req, res) => {
  try {
    const userId = req.user._id; // only fetch data of loggedIn user
    ...
    // fetch recent 10 tasks for loggedIn users
